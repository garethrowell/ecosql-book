# Ecological Data Management with PostgreSQL: 
open source tools for managing ecological change

1. Getting started with PostgreSQL
2. A generalized data structure for ecological monitoring 
3. Building your first ecological database
4. Ecological surveillance monitoring and climate response 
5. Using your database to quantify climate sensitivity
6. Sources of ecological and environmental data
7. Using data APIs in R with your database 
8. Data science and data publication in ecology
9. Using your database for ecological data publication 

